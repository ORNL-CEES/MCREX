...
%%---------------------------------------------------------------------------%%
\author{Stuart R. Slattery, Thomas M. Evans, Steven P. Hamilton, Paul
  P.H. Wilson
...
for these problems that simply reducing the spectral radius of the
Monte Carlo problem below unity is not enough to efficiently achieve
convergence as reported in previous work. In order to effectively
solve the $SP_N$ equations for the model problem, an algebraic
preconditioning strategy is developed for MCSA where we utilize
incomplete factorizations as preconditioners to achieve
convergence. Using the preconditioners, MCSA performance is compared
with production implementations of the preconditioned GMRES and
BiCGSTAB methods. Our studies indicate that the iterative performance
of MCSA when used to solve the $SP_N$ equations for light water
reactor problems is competitive with the preconditioned Krylov methods
...
application of the MCSA algorithm. Next, we demonstrate that the basic
Jacobi preconditioning applied in our previous work is unsatisfactory
to achieve convergence for neutron transport in light water reactors
and subsquently develop a new preconditioning scheme. We then apply
this preconditioning scheme to a light water reactor fuel assembly and
compare the performance of the preconditioned MCSA method to
preconditioned Krylov methods followed by a summary and conclusion.
...
\subsection{Fuel Assembly Model Problem}
\label{subsec:fuel_assembly}
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=6.5in]{problem3_axial_mat.png}
  \end{center}
...
\cite{evans_monte_2014} and therefore we apply it here to the $SP_N$
...
\begin{figure}[t!]
  \begin{center}
    \includegraphics[width=3.5in]{block_jacobi_res.pdf}
  \end{center}
...
using CPU time as a measure \cite{evans_monte_2014}.
