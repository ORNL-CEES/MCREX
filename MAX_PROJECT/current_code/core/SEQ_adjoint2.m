...
if ~ strcmp(fp.precond, 'alternating')
    ...
    if stat.adapt_walks==1 || stat.adapt_cutoff==1
        while(rel_residual>eps && count<=rich_it)
            ...
            [dx, dvar, X, resid, aux, rej]=MC_adjoint_adapt2(H, r, P, cdf, Pb, cdfb, stat);
    ...
    if stat.adapt_walks==1 || stat.adapt_cutoff==1
        while(rel_residual>eps && count<=rich_it)
            ...
            if mod(count,2)==1
                r=rhs1-B1*sol;
                [Pb, cdfb]=prob_adjoint_rhs(r, dist);
                [dx, dvar, resid, X, aux, rej]=MC_adjoint_adapt2(H1, r, P.P1, cdf.cdf1, Pb, cdfb, stat);
                NWALKS=[NWALKS size(X,1)];
                tally=[tally aux];
                reject=[reject rej];                
                sol=sol+dx;
                ...
                r=rhs2-B2*sol; 
                [Pb, cdfb]=prob_adjoint_rhs(r, dist);
                [dx, dvar, resid, X, aux, rej]=MC_adjoint_adapt2(H2, r, P.P2, cdf.cdf2, Pb, cdfb, stat);
                NWALKS=[NWALKS size(X,1)];
                tally=[tally aux];
                reject=[reject rej];           
                sol=sol+dx; 
