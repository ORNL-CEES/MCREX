function [sol, rel_res, VAR, REL_RES, DX, NWALKS, tally, count]=SEQ_adjoint1(fp, dist, P, cdf, numer, stat)
...
%     end  
    ...
    if stat.adapt_walks==1 || stat.adapt_cutoff==1
        while(rel_residual>eps && count<=rich_it)
            ...
            [dx, dvar, tot_walks, aux]=MC_adjoint_adapt(H, r, P, cdf, Pb, cdfb, stat);
    ...
    if stat.adapt_walks==1 || stat.adapt_cutoff==1
        while(rel_residual>eps && count<=rich_it)
            if mod(count,2)==1
                [Pb, cdfb]=prob_adjoint_rhs(r, dist);
                [dx, dvar, tot_walks, aux]=MC_adjoint_adapt(H1, r, P1, cdf1, Pb, cdfb, stat);
                NWALKS=[NWALKS tot_walks];
                tally=[tally aux];
                sol=sol+dx;
                r=rhs1-B1*sol;
                ...
                [Pb, cdfb]=prob_adjoint_rhs(r, dist);
                [dx, dvar, tot_walks, aux]=MC_adjoint_adapt(H2, r, P2, cdf2, Pb, cdfb, stat);
                NWALKS=[NWALKS tot_walks];
                tally=[tally aux];
                sol=sol+dx; 
                r=rhs2-B2*sol; 
