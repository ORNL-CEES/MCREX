%IT_SOLVE_POISSONCONTROL iterative solution of Poisson control problem
...
elseif precon==1,  % diagonal
  ...
  MA = 'mass_diagonal'; massparams = struct('M',M,'beta',beta);
...
elseif precon==2,  % block ideal, Schur comp. approx. 1
  ...
  MA = 'mass_exact'; massparams = struct('M',M,'beta',beta);
...
elseif precon==8,  % Chebyshev, Schur comp. approx. 1, GMG
  ...
  MA = 'mass_chebyshev';
  massparams = struct('Q',M,'its',cheb_its,'qmethod',qmethod,'beta',beta);
  if problem==4
      domain = 2; % L-shaped domain
  else
      domain = 1; % square domain
...
fprintf('\nMINRES iteration ...\n');
if precon==0, 
   [x_it,resvec,iter,flag] = poissoncontrol_minres(M,K,beta,Myhat,d, ...
       maxit,tol,MA,[],MS,[]);
else
   [x_it,resvec,iter,flag] = poissoncontrol_minres(M,K,beta,Myhat,d, ...
       maxit,tol,MA,massparams,MS,schurparams);
end
etoc = toc;
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
