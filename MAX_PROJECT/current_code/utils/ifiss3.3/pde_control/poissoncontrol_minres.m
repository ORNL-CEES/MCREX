function [x,resvec,iter,flag] = poissoncontrol_minres(M,K,beta,Myhat,d, ...
    maxit,tol,MA,aparams,MS,sparams)
% MINRES algorithm applied to the Poisson control matrix system
% [x,resvec,iter,flag] = poissoncontrol_minres(M,K,beta,Myhat,d, ...
%     maxit,tol,MA,aparams,MS,sparams)
%   input
%         M,K,beta        coefficient matrix [M,0,K;0,beta*M,-M;K,-M,0]
%         Myhat,d         RHS vector [Myhat;0;d]
...
fprintf('Call to POISSONCONTROL_MINRES ...\n')
Ahandle=str2func(MA);  Shandle=str2func(MS);

nm=size(M,1);
rhs=[Myhat;zeros(nm,1);d];x0=zeros(size(rhs));
Matrix=[M,sparse(nm,nm),K; sparse(nm,nm),beta*M,-M; K,-M,sparse(nm,nm)];
n=size(Matrix,1);
%
b=rhs;
x = x0;
res0=b-Matrix*x; 
res=zeros(n,1); 
res(1:2*nm)=Ahandle(res0(1:2*nm),aparams);  
res(2*nm+1:n)=Shandle(res0(2*nm+1:n),sparams);   
arr=sqrt( res0'*res); 
erro=arr;
resvec(1)=arr;
...
% Apply left precond P using P inner product for V.
     v(1:2*nm,1)=Ahandle(w1(1:2*nm),aparams);
     v(2*nm+1:n,1)=Shandle(w1(2*nm+1:n),sparams);
     H(i+1,i)=sqrt(v'*w1);
     if (H(i+1,i) ~= 0.)
