...
%kk=find(mboundr(:,2)==4); mboundr(kk,:)=[]; 
lastm=max(mboundr(find(mboundr(:,2)==4),1));
kkm=find(lastm==mboundr(:,1)& 4==mboundr(:,2));
mboundr(kkm-nkk+1:kkm,:)=[]; 
kkk=find(mboundr(:,2)==2);  mboundr(kkk,:)=[]; 
%
% reset boundary data
left=leftl; right=rightr+extra;
top=[topl(1:end-1);topr+extra]; 
