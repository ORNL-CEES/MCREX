...
C_tilde=filtering(C, 10^(-1));
H_tilde = sparse(speye(size(A_sp1,1)) - C_tilde*A_sp1);

display('\n Spectral radius of filtered iteration matrix --- left preconditioning:\n');
max(abs(eigs(H_tilde)))

Z2=filtering(Z, 5*10^(-2));
W2=filtering(W, 5*10^(-2));
C2=sparse(Z2 * D_inv * W2);
H2 = sparse(speye(size(A_sp1,1)) - C2*A_sp1);
...
H3 = sparse(speye(size(A_sp1,1)) - C3*A_sp1);
max(abs(eigs(H3)))

H4 = speye(size(A_sp1,1)) - sparse(D_inv*W*A_sp1*Z);
